...
Use this reference when creating or improving `.agents/skills/{name}/SKILL.md`.

## 1. Purpose

A Codex skill contains reusable process knowledge: procedures, checklists, output formats, domain rules, reference-loading instructions, and validation criteria. One-off task details belong in the orchestrator prompt or runtime task record, not permanently in the skill.

Skills should be Codex-native:
...
Use short sections with concrete actions. Do not repeat project-level rules that belong in `AGENTS.md`.
...
| Codex-native     | Use Codex files, skills, agents, and available delegation rules; do not reference unsupported tools.                          |
...
The skill body should say when to load each reference.

## 9. Do Not Include

- one-off user task details
- secrets, credentials, or private tokens
- obsolete Claude-only tool calls
