...
6. Load `recipe-agent-team-architecture-design` when the workflow needs backend design artifacts before coding.
7. Load `persona-agent-team-designer` when the workflow needs visual/UI/icon/character/environment/logo/design-system brief or spec work.
8. Load `recipe-agent-team-run-lifecycle`, then resolve or create the orchestrator-owned runtime context for orchestrated harness execution.
...
10. Execute direct work or delegate bounded specialist tasks when allowed.
11. Collect returned summaries, task records, inbox messages, evidence, and artifacts.
12. Load `recipe-agent-team-compound-learning` when the workflow produced reusable learning for future runs.
13. Retry, block, request clarification, or complete.
14. Report final artifacts and unresolved risks.
```
