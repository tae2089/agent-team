...
Use when discovery selects subdomain `design-system`. Produces a reusable token catalog as a single `DESIGN.md` file plus optional supporting docs. Downstream subdomains (`ui`, `logo-branding`, `icon-illustration`) cite this catalog instead of inlining values.
...
## When To Use This Subdomain

Pick `design-system` when at least one is true:
...
Skip `design-system` and pick a surface subdomain directly when only one screen, one logo, or one icon set is in scope and tokens will not be reused.

## Output Layout

```
_workspace/{run_id}/design/design-system/
...
brief: "_workspace/{run_id}/design/design-system/design-brief.md"
...
2. Frontmatter includes `routed_by`, `subdomain`, `brief`, at least `colors`, `typography`, `spacing`, and one `components` entry. `rounded` and `elevation` may be empty groups with a recorded reason.
...
- Do not embed pixel-perfect mockups or rendered assets — this subdomain produces tokens and rationale, not visuals.
...
- Start with `colors` and `typography`; downstream `ui`/`logo-branding`/`icon-illustration` references depend on them most heavily.
- When a downstream surface needs a value the system does not yet have, add a token to `DESIGN.md` rather than inlining; rerun this subdomain to update.
...
|---|---|
| Tokens ready, screens needed | Run `recipe-agent-team-design-interview` with `subdomain: ui`, then `recipe-agent-team-design-spec`; cite `DESIGN.md` in Upstream Inputs |
| Tokens ready, logo needed | Run `recipe-agent-team-design-interview` with `subdomain: logo-branding`, then `recipe-agent-team-design-spec`; cite `DESIGN.md` in Upstream Inputs |
| Tokens ready, icon system needed | Run `recipe-agent-team-design-interview` with `subdomain: icon-illustration`, then `recipe-agent-team-design-spec`; cite `DESIGN.md` in Upstream Inputs |
...
| Downstream surface needs a value not in tokens | Pause that surface; rerun this subdomain to add the missing token |
